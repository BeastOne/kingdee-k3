/**
 * A interface for the refund bill.
 */
"use strict";
...
var
  _ = require('lodash'),
  utils = require('../utils'),
  json_save = require('../../tpl/ar-refundbill-save.json');

var RefundBill = function () {
  this.data = _.cloneDeep(json_save);
  this.entitySchema = this.data.Model.FREFUNDBILLENTRY[0];
  this.data.Model.FREFUNDBILLENTRY = [];
  this.data.Model.FDATE = utils.date2Str(new Date(), 'yyyy-MM-dd hh:mm:ss');
}

RefundBill.prototype.assign = function (bill) {
  ...
  if (bill.FEntities) {
    ...
    for (var i = 0; i < bill.FEntities.length; i++) {
      ...
      this.data.Model.FREFUNDBILLENTRY.push(entity)
    }
  }
}
...
module.exports.audit = function (client, numbers, createOrgId) {
  ...
  return client.audit('AR_REFUNDBILL', data);
}
...
module.exports.save = function (client, bill) {
  var data = new RefundBill();
  data.assign(bill);

  return client.save('AR_REFUNDBILL', data.data);
}
...
module.exports.submit = function (client, numbers, createOrgId) {
  ...
  return client.submit('AR_REFUNDBILL', data);
}

/**
 * 财务会计 -> 出纳管理 -> 收款退款单 -> 保存 -> 提交 -> 审核
 * @param {K3CloudApiClient} client
 * @param {Object} bill
 */
